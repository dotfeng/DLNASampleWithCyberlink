...
public class SearchThread extends Thread {
	...
	private void searchDevices() {
		try {
			if (mStartComplete) {
				...
				DebugLog.d("controlpoint search...");
			} else {
				mControlPoint.stop();
				boolean startRet = mControlPoint.start();
				DebugLog.d("controlpoint start:" + startRet);
				if (startRet) {
					mStartComplete = true;
				}
			}
	...
	private DeviceChangeListener mDeviceChangeListener = new DeviceChangeListener() {
		...
		public void deviceRemoved(Device dev) {
			DebugLog.d("control point remove a device");
			ControlPointContainer.getInstance().removeDevice(dev);
		}

		@Override
		public void deviceAdded(Device dev) {
			DebugLog.d("control point add a device..." + dev.getDeviceType() + dev.getFriendlyName());
