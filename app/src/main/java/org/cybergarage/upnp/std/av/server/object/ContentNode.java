...
//edit feng move from containerNode
////////////////////////////////////////////////
//	findContentNodeBy*
////////////////////////////////////////////////
	...
	public ContentNode findContentNodeByName(String name, long size) {
		...
		for (int n = 0; n < nodeCnt; n++) {
			...
			if (cnode.isItemNode()) {
				String nodname = cnode.getName();
				long nodsize = ((ItemNode) cnode).getStorageUsed();
				if (name.equals(nodname) && size == nodsize) {
					return cnode;
				}

			} else {
				ContentNode fnode = cnode.findContentNodeByName(name, size);
				if (fnode != null)
					return fnode;
			}
		}
